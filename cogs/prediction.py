...
class Prediction(commands.Cog):
    ...
    async def get_type_and_region_pingers_for_spawn(
        self, pokemon_name: str, guild_id: int
    ) -> tuple[list, list]:
        """
        Fetch type AND region pingers in a single DB call for get_type_region_afk_users.
        Returns (type_pingers, region_pingers).
        """
        types   = get_pokemon_types(pokemon_name)
        regions = get_pokemon_region(pokemon_name)

        # If neither has data, skip the DB round-trip entirely
        if not types and not regions:
            return [], []

        # ONE shared DB call instead of two
        afk_map = await self.db.get_type_region_afk_users()
        type_afk   = {uid for uid, flags in afk_map.items() if flags.get('type')}
        region_afk = {uid for uid, flags in afk_map.items() if flags.get('region')}

        # Fire both DB queries in parallel now that we have the afk sets
        tasks = []
        if types:
            tasks.append(self.db.get_users_for_types(guild_id, types, type_afk))
        if regions:
            tasks.append(self.db.get_users_for_regions(guild_id, regions, region_afk))

        results = await asyncio.gather(*tasks)

        if types and regions:
    ...
    async def build_prediction_output(
        ...
        *,
        ...
        tasks = [
            ...
            self.get_type_and_region_pingers_for_spawn(name, guild_id), # 3 — single DB call
        ]
        ...
        type_region = results[3] if isinstance(results[3], tuple) else ([], [])
        type_pingers, rgn_pingers = type_region
                                ...
                                if name and confidence:
                                    ...
                                    try:
                                        ...
                                        tasks = [
                                            ...
                                            self.get_type_and_region_pingers_for_spawn(name, message.guild.id),
                                        ]
                                        results = await asyncio.gather(*tasks, return_exceptions=True)
                                        hunters    = results[0] if isinstance(results[0], list) else []
                                        collectors = results[1] if isinstance(results[1], list) else []
